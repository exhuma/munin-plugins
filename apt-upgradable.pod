...
The values are cached in '/var/lib/munin-node/apt_upgradable'. This folder
needs to exist and needs to be writable by the user as which this plugin runs.
Likely 'nobody' or 'root'. The plugin will try to create the folder if it's
missing, but during my tests, I needed a manual step to create it!
...
The plugin assumes that aptitude is available under /usr/bin/aptitude. If it's
not available in that location, you can override it with the config variable
'aptitude_path'

You can change the colors if you want using 'essential_color' and
'upgrades_color'
